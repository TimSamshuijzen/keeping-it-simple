...
When processing a change request, augment the requirements in 
`docs/requirements.md` to reflect the change request, and then remove the 
change request from this document. A single change request can result in 
multiple new requirements, and/or changes in existing requirements. Ensure 
that the added requirements describe the acceptance criteria.


# Change requests

The change requests table below lists the change requests. When making changes 
to the table, make sure the table format remains intact.
...
| ID | Change request description | Architect notes |
| --- | --- | --- |
